...
if (( $+functions[compdef] )); then
  _backscroll() {
    if (( CURRENT == 2 )); then
      compadd run exec init list last show search pick diff export import sync stats note prune delete redact mcp serve off on doctor upgrade version help
